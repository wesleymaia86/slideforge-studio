...
import { Download, FileText, Film, Globe, Clock, CheckCircle2, Loader2, Plus } from 'lucide-react'
import { TopBar } from '@/components/layout/TopBar'
import { useExports, useCreateExport } from '@/lib/api/hooks'
import { mockExports } from '@/lib/mocks'
...
const FORMAT_OPTIONS = [
  {
    ...
    label: 'PowerPoint',
    ext: '.pptx',
    description: 'Editable presentation for Microsoft PowerPoint',
  ...
  },
  {
    ...
    label: 'PDF',
    ext: '.pdf',
    description: 'Fixed-layout document, ideal for distribution',
    icon: <FileText className="w-6 h-6" />,
  ...
  },
  {
    ...
    ext: '.html',
    description: 'Interactive browser-based presentation',
  ...
  },
  {
    ...
    label: 'PNG Images',
    ext: '.zip',
    description: 'Export each slide as a high-res image',
...
export default function ExportsPage() {
  ...
  const display = exports ?? mockExports
        ...
        <div className="mb-8">
          ...
          <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-3 mb-5">
            {FORMAT_OPTIONS.map((fmt) => (
              <div
                key={fmt.id}
                className={`bg-gradient-to-br ${fmt.gradient} border ${fmt.border} rounded-xl p-4 flex flex-col gap-3`}
              >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 h-8 bg-surface/60 border border-border/50 rounded-lg text-xs font-medium text-text hover:bg-surface-2 transition-colors disabled:opacity-60 mt-auto"
                >
                  {createExport.isPending && createExport.variables?.format === fmt.id ? (
                    <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Creating…</>
                  ) : (
                    <><Download className="w-3.5 h-3.5" /> Export {fmt.ext}</>
                  )}
                </button>
              </div>
            ))}
                ...
                <div key={exp.id} className="flex items-center gap-4 px-4 py-3 bg-surface rounded-xl border border-border">
                  ...
                  {exp.status === 'ready' && exp.url ? (
                    <a
                      href={exp.url}
                      download
                      className="flex items-center gap-2 h-8 px-3 bg-accent text-[#0C0D0F] font-semibold text-xs rounded-lg hover:bg-accent-light transition-colors shadow-amber-sm"
                    >
