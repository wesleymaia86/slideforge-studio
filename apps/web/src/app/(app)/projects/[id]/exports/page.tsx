...
const FORMAT_OPTIONS = [
  { id: 'pptx' as const, label: t('exports.pptx'), ext: '.pptx', description: t('exports.pptxDesc'), icon: <FileText className="w-6 h-6" />, gradient: 'from-orange-500/20 to-orange-500/5', border: 'border-orange-500/25', color: 'text-orange-400' },
  { id: 'pdf' as const, label: t('exports.pdf'), ext: '.pdf', description: t('exports.pdfDesc'), icon: <FileText className="w-6 h-6" />, gradient: 'from-red-500/20 to-red-500/5', border: 'border-red-500/25', color: 'text-red-400' },
  { id: 'html' as const, label: t('exports.html'), ext: '.html', description: t('exports.htmlDesc'), icon: <Globe className="w-6 h-6" />, gradient: 'from-info/20 to-info/5', border: 'border-info/25', color: 'text-info' },
  { id: 'png' as const, label: t('exports.png'), ext: '.zip', description: t('exports.pngDesc'), icon: <Film className="w-6 h-6" />, gradient: 'from-success/20 to-success/5', border: 'border-success/25', color: 'text-success' },
]
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        <div className="mb-6">
          <div className="flex items-center gap-2.5 mb-2">
            ...
            <h1 className="font-display text-2xl text-text">{t('exports.title')}</h1>
          </div>
          <p className="text-text-muted text-sm">Baixe sua apresentação em qualquer formato.</p>
        </div>

        <div className="mb-8">
          <h2 className="text-sm font-semibold text-text mb-4">{t('exports.chooseFormat')}</h2>
          ...
          <div className="flex items-center gap-6 flex-wrap">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-xs text-text-muted">Incluir notas do apresentador</span>
            </label>
          </div>
        </div>

        {isLoading ? (
          <p className="text-sm text-text-muted">Carregando exportações…</p>
        ) : display.length === 0 ? (
          <EmptyState compact title={t('exports.noExports')} description={t('exports.noExportsDesc')} />
        ) : (
          <div>
            <h2 className="text-sm font-semibold text-text mb-3">Exportações anteriores</h2>
                ...
                <div key={exp.id} className="flex items-center gap-4 px-4 py-3 bg-surface rounded-xl border border-border">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-text-faint flex items-center gap-1 mt-0.5">
                      ...
                      {new Date(exp.createdAt).toLocaleDateString('pt-BR', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </p>
                  </div>
